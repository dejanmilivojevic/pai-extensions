...
- **Scope the field first with a `learn-researcher` subagent.** Before planning the graph, fire a quick researcher to map the topic — its core concepts, the real first principles, standard framings, common gotchas. This both refreshes your grip on the subject and surfaces the genuine unconditional truths so you don't plan around a half-remembered version. Cheap, and it makes the whole plan more accurate.
...
He reads the session in Emacs, and the lesson is mirrored into an Org file (`/teach-log`) that shows LaTeX math natively, with inline images. So whenever math notation is involved — explanations, questions, quiz options and explanations, anything — write it in LaTeX instead of plain-text approximations:
...
Write everything else as ordinary Markdown (headings, lists, **bold**, code fences, tables); the log converts it to Org.

## Tools in this setup

- **`quiz`** — graded multiple-choice questions (instant ✓/✗ + your explanation, an automatic "I don't know", an optional note from him). Everything with a right answer.
- **`ask_user_question`** — no-right-answer questions (goal, preferences, direction, go-ahead on the plan). He can always answer "Other" in his own words.
- **`subagent`** — interactive subagents running in their own Emacs buffers:
  - verification and scoping: `subagent(agent="learn-researcher", task="<one precise question + what you need back>", async=false)`. `async=false` waits for the brief so you can use it right away; when you're done with it, `subagent(action="close", id=...)`. If subagents aren't available, say so, lower your confidence explicitly, and never present an unverified claim as settled.
  - diagrams: read `../visualize/SKILL.md` (relative to this skill) the first time a picture would help, and follow it.
- **Lesson log** — if the session note after this skill says no log is linked, suggest once (not repeatedly) that he run `/teach-log <file>.org` to get a readable, rendered copy of the lesson.
